...
def home():
    ...
    return {
        ...
        "endpoints_disponiveis": {
            "POST /Login": "Realizar login por perfil",
...
def cadastro_cliente(cliente: schemas.ClienteCreate, db: Session = Depends(get_db)):
    ...
    # Extrai a string pura da senha do SecretStr
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, 
            detail=f"Erro interno ao salvar no banco. Detalhe: {str(e)}"
        )
...
def atualizar_clientes(cliente_id: int, cliente_update: schemas.ClienteUpdate, db: Session = Depends(get_db)):
    ...
    # CORREÇÃO: Trata a instância do SecretStr se a senha for enviada no update
    if "senha_cliente" in update_data and isinstance(update_data["senha_cliente"], SecretStr):
        update_data["senha_cliente"] = update_data["senha_cliente"].get_secret_value()
...
def atualizar_profissionais(profissional_id: int, profissional_update: schemas.ProfissionalUpdate, db: Session = Depends(get_db)):
    ...
    # CORREÇÃO: Trata a instância do SecretStr se a senha for enviada no update
    if "senha_profissional" in update_data and isinstance(update_data["senha_profissional"], SecretStr):
        update_data["senha_profissional"] = update_data["senha_profissional"].get_secret_value()
...
# Mantém a pasta static servindo o frontend na raiz
